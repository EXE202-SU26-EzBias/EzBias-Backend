...
public interface IPaymentApplicationService
{
    ...
    Task<(bool Success, string? Error)> HandleWebhookAsync(PaymentWebhookRequest request, string rawBody, string? signature, CancellationToken ct);
    Task<(bool Success, string? Error)> HandleSePayWebhookAsync(SePayWebhookPayload payload, string rawBody, string? signature, CancellationToken ct);
